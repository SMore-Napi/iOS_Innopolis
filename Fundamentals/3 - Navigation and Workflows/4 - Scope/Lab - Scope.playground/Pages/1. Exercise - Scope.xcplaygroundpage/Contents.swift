...
//print("The value of foo is \(foo)")
//The constant 'foo' is declared inside the scope of the for loop. So, outside of this loop the constant is not visible
...
// 'x' is defined in a global scope and modified in a local scope.
// This variable can be seen for both 'print(_:)' functions (in a nested local scope and in the same gloabl scope).
...
func greeting(greeting: String?, name: String) {
    if let greetling = greeting {
        print("\(greetling) \(name)")
    } else {
        print("Hello \(name)")
    }
}
